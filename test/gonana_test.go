...
func TestIsSnakeCase(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := types.IsSnakeCase(tt.input)
			if result != tt.expected {
				t.Errorf("types.IsSnakeCase(%q) = %v, want %v", tt.input, result, tt.expected)
			}
		})
	}
}
...
func TestIsScreamingSnakeCase(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := types.IsScreamingSnakeCase(tt.input)
			if result != tt.expected {
				t.Errorf("types.IsScreamingSnakeCase(%q) = %v, want %v", tt.input, result, tt.expected)
			}
		})
	}
}
...
func TestExtractFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			functions := types.ExtractFunctions(tt.lines)
			if len(functions) != tt.expected {
				t.Errorf("types.ExtractFunctions() returned %d functions, want %d", len(functions), tt.expected)
			}
			if tt.expected > 0 && len(functions) > 0 {
				if functions[0].Name != tt.funcName {
					t.Errorf("types.ExtractFunctions() first function name = %q, want %q", functions[0].Name, tt.funcName)
				}
			}
		})
	}
...
func TestCheckLineLength(t *testing.T) {
	...
	violations := rules.CheckLineLength(analysis, "test.c", 0)
	if len(violations) != 1 {
		t.Errorf("rules.CheckLineLength() found %d violations, want 1", len(violations))
	}
	if len(violations) > 0 && violations[0].Rule != "C-L1" {
		t.Errorf("rules.CheckLineLength() violation rule = %q, want %q", violations[0].Rule, "C-L1")
	}
	if len(violations) > 0 && violations[0].Line != 2 {
		t.Errorf("rules.CheckLineLength() violation line = %d, want 2", violations[0].Line)
	}
}
...
func TestCheckEmptyLines(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Lines: tt.lines}
			violations := rules.CheckEmptyLines(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckEmptyLines() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckIndentation(t *testing.T) {
	...
	violations := rules.CheckIndentation(analysis, "test.c", 0)
	if len(violations) != 1 {
		t.Errorf("rules.CheckIndentation() found %d violations, want 1", len(violations))
	}
	if len(violations) > 0 && violations[0].Rule != "C-L3" {
		t.Errorf("rules.CheckIndentation() violation rule = %q, want %q", violations[0].Rule, "C-L3")
	}
}
...
func TestCheckVariableDeclaration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Lines: tt.lines}
			violations := rules.CheckVariableDeclaration(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckVariableDeclaration() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckFilename(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{}
			violations := rules.CheckFilename(analysis, tt.filename, 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckFilename(%q) found %d violations, want %d", tt.filename, len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckFunctionNames(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Functions: tt.functions}
			violations := rules.CheckFunctionNames(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckFunctionNames() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckMacroNames(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Lines: tt.lines}
			violations := rules.CheckMacroNames(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckMacroNames() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckFunctionLength(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Functions: tt.functions}
			violations := rules.CheckFunctionLength(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckFunctionLength() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckCommentFormat(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Lines: tt.lines}
			violations := rules.CheckCommentFormat(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckCommentFormat() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckFunctionParameters(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Functions: tt.functions}
			violations := rules.CheckFunctionParameters(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckFunctionParameters() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestCheckForLoopDeclaration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Lines: tt.lines}
			violations := rules.CheckForLoopDeclaration(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckForLoopDeclaration() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestNewAnalyzer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analyzer := analyzer.NewAnalyzer(tt.level)
			if analyzer.Level() != tt.level {
				t.Errorf("analyzer.NewAnalyzer(%d).level = %d, want %d", tt.level, analyzer.Level(), tt.level)
			}
			if len(analyzer.Rules()) != tt.expectedRules {
				t.Errorf("analyzer.NewAnalyzer(%d) has %d rules, want %d", tt.level, len(analyzer.Rules()), tt.expectedRules)
			}
		})
	}
}
...
`
	...
	analyzer := analyzer.NewAnalyzer(1)
...
func TestExtractFunctions_WithParameters(t *testing.T) {
	...
	if len(functions) != 1 {
		t.Fatalf("types.ExtractFunctions() returned %d functions, want 1", len(functions))
	}

	if functions[0].ParamCount != 4 {
		t.Errorf("types.ExtractFunctions() param count = %d, want 4", functions[0].ParamCount)
	}

	if functions[0].Name != "add" {
		t.Errorf("types.ExtractFunctions() function name = %q, want %q", functions[0].Name, "add")
	}
}
...
func TestExtractFunctions_VoidParams(t *testing.T) {
	...
	if functions[0].ParamCount != 0 {
		t.Errorf("types.ExtractFunctions() param count = %d, want 0 for void", functions[0].ParamCount)
	}
}

/* TestGetProgressBar - commented out, function removed in refactoring
...
`
	...
	// Test with level 1
	analyzer1 := analyzer.NewAnalyzer(1)
	...
	// Test with level 2
	analyzer2 := analyzer.NewAnalyzer(2)
...
/* TestOutputJSON - commented out, tests unexported function in main
...
func TestPrintReport(t *testing.T) {
	...
	reporter.PrintReport(report, false)
...
func TestPrintReportVerbose(t *testing.T) {
	...
	reporter.PrintReport(report, true)
...
func TestCheckFunctionCount_EdgeCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			analysis := &types.FileAnalysis{Lines: tt.lines}
			violations := rules.CheckFunctionCount(analysis, "test.c", 0)
			if len(violations) != tt.expected {
				t.Errorf("rules.CheckFunctionCount() found %d violations, want %d", len(violations), tt.expected)
			}
		})
	}
}
...
func TestExtractFunctions_ComplexCases(t *testing.T) {
	tests := []struct {
		...
		lines         []string
		expectedCount int
		checkFunc     func(*testing.T, []types.FunctionInfo)
	}{
		{
			...
			expectedCount: 1,
			checkFunc: func(t *testing.T, fns []types.FunctionInfo) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(functions) != tt.expectedCount {
				t.Errorf("types.ExtractFunctions() returned %d functions, want %d", len(functions), tt.expectedCount)
			}
...
/* TestPrintHeader - commented out, tests unexported function
...
/* TestPrintSummary - commented out, tests unexported function
...
func TestCollectCFiles_SingleFile(t *testing.T) {
	...
	files, err := types.CollectCFiles(testFile)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCollectCFiles_Directory(t *testing.T) {
	...
	files, err := types.CollectCFiles(tmpDir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCollectCFiles_InvalidPath(t *testing.T) {
	_, err := types.CollectCFiles("/nonexistent/path")
	if err == nil {
		t.Error("Expected error for nonexistent path")
	}
}

/* TestRunFixer tests - commented out, test unexported main function
func TestRunFixer_NoCFiles(t *testing.T) {
	...
	analyzer := analyzer.NewAnalyzer(1)
	fixer := fixer.NewFixer(analyzer, true)
...
`
	...
	analyzer := analyzer.NewAnalyzer(1)
	fixer := fixer.NewFixer(analyzer, false)
...
/* TestPrintFileResults_LongFilename - commented out, tests unexported reporter function
		...
		Files: []types.FileResult{
			{
				...
				Score:      85.5,
				LineCount:  100,
				Violations: []types.Violation{{Rule: "C-L1", Line: 1, Message: "Test"}},
			},
		},
		TotalFiles: 1,
		TotalLines: 100,
...
func TestCalculateScore_AllViolationTypes(t *testing.T) {
	analyzer := analyzer.NewAnalyzer(2)
...
/* TestFix*_EdgeCases - commented out, test unexported fixer methods
func TestFixEmptyLines_EdgeCases(t *testing.T) {
	fixer := fixer.NewFixer(nil, true)
...
func TestToSnakeCase_SpecialCases(t *testing.T) {
	...
	for _, tt := range tests {
		result := types.ToSnakeCase(tt.input)
		if result != tt.expected {
